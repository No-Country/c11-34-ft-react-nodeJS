...
export function Sidebar({isOpen, onOpen}) {
  ...
  return (
    <div className={`${isOpen ? 'right-0' : '-right-full' } bg-black h-screen fixed top-0 w-96 text-white font-inter z-50 flex flex-col gap-y-4  py-20 transition-all duration-500 ease-in-out`}>
        ...
        <Link className='hover:bg-gray-200 hover:text-black transition-all py-1.5 px-6 ' to={`${!isAuth ? '/auth' : '/my-perfil'}`}>Favoritos</Link>
